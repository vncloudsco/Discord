namespace ICSharpCode.SharpZipLib.Zip
{
    ...
    internal sealed class ZipExtraData : IDisposable
    {
        ...
        {
            this.AddLeInt((int) (((ulong) toAdd) & 0xffffffffUL));
        ...
        private static ITaggedData Create(short tag, byte[] data, int offset, int count)
        {
            ...
            data2 = (tag == 10) ? ((ITaggedData) new NTTaggedData()) : ((tag == 0x5455) ? ((ITaggedData) new ExtendedUnixData()) : ((ITaggedData) new RawTaggedData(tag)));
        ...
        public bool Find(int headerID)
        {
            ...
            bool local1 = (num2 == headerID) && ((this._index + num) <= this._data.Length);
            if (local1)
            {
        ...
        public int ReadInt()
        {
            this.ReadCheck(4);
            this._index += 4;
            return (((this._data[this._index] + (this._data[this._index + 1] << 8)) + (this._data[this._index + 2] << 0x10)) + (this._data[this._index + 3] << 0x18));
        }

        public long ReadLong()
        {
            this.ReadCheck(8);
            return ((this.ReadInt() & 0xffffffffUL) | (this.ReadInt() << 0x20));
        }
        ...
        public int ReadShort()
        {
            this.ReadCheck(2);
            this._index += 2;
            return (this._data[this._index] + (this._data[this._index + 1] << 8));
        }
